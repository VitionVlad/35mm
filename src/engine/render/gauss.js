...
class Gaussh{
    constructor(canvasid){
        ...
        this.deffered = device.createTexture({
            label: "deff",
            format: deffformat,
            size: [this.rres[0], this.rres[1], this.defferedcnt*3],
            usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.RENDER_ATTACHMENT,
        });
        ...
        this.deffbf = new Float32Array(400);
...
class Gausmesh{
    ...
    drawsh(eh, i){
        ...
        this.ubo[7] = gs.handle[eh].lightscount;
...
export function gamepad_axisnm(){
    return gpaxes.length;
}
export function gamepad_buttonnm(){
    return gpbtns.length;
}
...
export function modifyshadowdata(eh, ncnt, nres, lcnt){
    gs.handle[eh].lightscount = lcnt;
    if(ncnt === 0){
        gs.handle[eh].enableshadowmaps = false;
        gs.handle[eh].shadowmapcnt = 1;
        gs.handle[eh].shadowmapresolution = 1;
    }else{
        gs.handle[eh].enableshadowmaps = true;
...
export function rn(eh){
    ...
    if(gs.handle[eh].enableshadowmaps){
